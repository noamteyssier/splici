...
fn populate_introns(transcript_records: &HashMap<usize, Vec<ExonRecord>>, extension: Option<usize>) -> HashMap<usize, Vec<GenomicInterval<usize>>> {
    transcript_records
        ...
        .map(|exon_set| {
            ...
            let mut exon_set = exon_set.iter()
                .map(|exon| exon.into())
                .collect::<GenomicIntervalSet<usize>>();
            exon_set.sort();

            // calculate the introns
            let intron_set = exon_set.internal()
        ...
        .fold(HashMap::new(), |mut gene_introns, (parent_id, intron_set)| {
            intron_set
                .for_each(|intron| {
                    gene_introns.entry(parent_id)
                        .or_insert_with(Vec::new)
                        .push(intron);
                });
            gene_introns
...
fn merge_introns(gene_introns: &HashMap<usize, Vec<GenomicInterval<usize>>>) -> HashMap<usize, GenomicIntervalSet<usize>> {
    gene_introns
        .into_iter()
...
fn main() -> Result<()> {
    ...
    for transcript in transcript_records.keys() {
        let full_transcript_name = transcript_id_map.get(transcript)
            .map(|x| from_utf8(x)).unwrap()?;
        let exon_set = transcript_records.get(transcript).unwrap();
        let mut exon_set = exon_set.iter().map(|x| x.into()).collect::<GenomicIntervalSet<usize>>();
        exon_set.sort();
        let transcript_seq = exon_set.records().iter().map(|x| {
